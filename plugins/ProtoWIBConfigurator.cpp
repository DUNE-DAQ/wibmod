...
ProtoWIBConfigurator::do_settings(const data_t& payload)
{
  ...
  if (daqMode == WIB::RCE) {
    ...
    if (expected_daq_mode != "RCE" && expected_daq_mode != "rce" && expected_daq_mode != "ANY" &&
    ...
    if (expected_daq_mode != "FELIX" && expected_daq_mode != "felix" && expected_daq_mode != "ANY" &&
        expected_daq_mode != "any") {
  ...
  TLOG_DEBUG(0) << "WIB Firmware Version: 0x" << std::hex << std::setw(8) << std::setfill('0') << wib_fw_version
                << " Synthesized: " << std::hex << std::setw(2) << std::setfill('0')
                << wib->Read("SYSTEM.SYNTH_DATE.CENTURY") << std::hex << std::setw(2) << std::setfill('0')
                << wib->Read("SYSTEM.SYNTH_DATE.YEAR") << "-" << std::hex << std::setw(2) << std::setfill('0')
                << wib->Read("SYSTEM.SYNTH_DATE.MONTH") << "-" << std::hex << std::setw(2) << std::setfill('0')
                << wib->Read("SYSTEM.SYNTH_DATE.DAY") << " " << std::hex << std::setw(2) << std::setfill('0')
                << wib->Read("SYSTEM.SYNTH_TIME.HOUR") << ":" << std::hex << std::setw(2) << std::setfill('0')
                << wib->Read("SYSTEM.SYNTH_TIME.MINUTE") << ":" << std::hex << std::setw(2) << std::setfill('0')
  ...
  TLOG_DEBUG(0) << "Enabling DAQ links " << conf.link1_enabled << " " << conf.link2_enabled << " " << conf.link3_enabled
                << " " << conf.link4_enabled;
...
void
ProtoWIBConfigurator::setup_femb_fake_data(size_t iFEMB,
                                           ...
                                           bool continue_on_reg_read_error)
{
  ...
  if (wib->ReadFEMB(iFEMB, "VERSION_ID") == wib->ReadFEMB(iFEMB, "SYS_RESET")) { // can't read register if equal
    if (continue_on_reg_read_error) {
      ers::warning(CannotReadFromFEMB(
        ERS_HERE, get_name(), "Can't read registers from FEMB" + std::to_string(iFEMB) + "; power down and continue"));
  ...
  if (expected_femb_fw_version != femb_fw_version) {
    throw IncorrectFirmwareVersion(
      ERS_HERE, get_name(), "FEMB" + std::to_string(iFEMB), femb_fw_version, expected_femb_fw_version);
  }
  ...
  if (fakeDataSelect == "fake_word") {
    ...
    if (fake_waveform.size() != 255) {
      ...
      excpt << "setup_femb_fake_data: FEMB " << iFEMB << " fake_waveform is not implemented!";
    ...
    std::stringstream excpt;
    excpt << "FEMB" << iFEMB << " fake_data_select is \"" << fakeDataSelect << "\" but expect "
...
void
ProtoWIBConfigurator::setup_femb(size_t iFEMB,
                                 const protowibconfigurator::FEMBSettings& FEMB_conf,
                                 bool continue_on_reg_read_error)
{
  if (FEMB_conf.gain > 3) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " gain shouldn't be larger than 3 is: " << FEMB_conf.gain;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.shape > 3) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " shape shouldn't be larger than 3 is: " << FEMB_conf.shape;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.baseline_high > 2) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " baseline_high should be 0 or 1 or 2 is: " << FEMB_conf.baseline_high;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.leak_high > 1) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " leak_high should be 0 or 1 is: " << FEMB_conf.leak_high;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.leak_10x > 1) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " leak_10x should be 0 or 1 is: " << FEMB_conf.leak_10x;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.ac_couple > 1) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " ac_couple should be 0 or 1 is: " << FEMB_conf.ac_couple;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.buffer > 1) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " buffer should be 0 or 1 is: " << FEMB_conf.buffer;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.ext_clk > 1) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " ext_clk should be 0 or 1 is: " << FEMB_conf.ext_clk;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.clk_phases.size() == 0) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " clk_phases size should be > 0 ";
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.pulse_mode > 2) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB
          << " pulse_mode should be 0 (off) 1 (FE ASIC internal) or 2 (FPGA external) is: " << FEMB_conf.pulse_mode;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if ((FEMB_conf.pulse_mode == 1 && FEMB_conf.pulse_dac > 63) ||
      (FEMB_conf.pulse_mode == 2 && FEMB_conf.pulse_dac > 31)) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " pulse_dac should be 0-31 in pulse_mode 1 or 0-63 in pulse_mode 2."
          << " pulse_mode is " << FEMB_conf.pulse_mode << " pulse_dac is " << FEMB_conf.pulse_dac;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.start_frame_mode > 1) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " start_frame_mode should be 0 or 1 is: " << FEMB_conf.start_frame_mode;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  if (FEMB_conf.start_frame_swap > 1) {
    std::stringstream excpt;
    excpt << "setup_femb: FEMB " << iFEMB << " start_frame_swap should be 0 or 1 is: " << FEMB_conf.start_frame_swap;
    throw InvalidFEMBSetting(ERS_HERE, get_name(), excpt.str());
  }
  ...
  if (FEMB_conf.expected_femb_fw_version != femb_fw_version) {
    throw IncorrectFirmwareVersion(
      ERS_HERE, get_name(), "FEMB" + std::to_string(iFEMB), femb_fw_version, FEMB_conf.expected_femb_fw_version);
  }

  std::vector<uint32_t> fe_config = { FEMB_conf.gain,      FEMB_conf.shape,    FEMB_conf.baseline_high,
                                      FEMB_conf.leak_high, FEMB_conf.leak_10x, FEMB_conf.ac_couple,
                                      FEMB_conf.buffer,    FEMB_conf.ext_clk };

  wib->ConfigFEMB(iFEMB,
                  fe_config,
                  FEMB_conf.clk_phases,
...
ProtoWIBConfigurator::do_scrap(const data_t&)
{
  ...
  if (wib->GetDAQMode() == WIB::FELIX) {
    ...
    if (!success) {
      ers::warning(CommandFailed(
        ERS_HERE, get_name(), "Failed to stop data transmission after " + std::to_string(stop_run_tries) + " tries"));
    }
  } // if felix
  wib = NULL;
