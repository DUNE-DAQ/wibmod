...
void
WIB::WriteDAQ_SI5342(uint16_t address, uint32_t value, uint8_t byte_count)
{
  WriteI2C("DAQ.SI5342.I2C", address, value, byte_count);
}
uint32_t
WIB::ReadDAQ_SI5342(uint16_t address, uint8_t byte_count)
{
...
void
WIB::SetDAQ_SI5342Page(uint8_t page)
{
...
uint8_t
WIB::GetDAQ_SI5342AddressPage(uint16_t address)
{
  return uint8_t((address >> 8) & 0xFF);
}

void
WIB::LoadConfigDAQ_SI5342(std::string const& fileName)
{
...
void
WIB::SelectSI5342(uint64_t input, bool enable)
{
