...
void
WIB::EnableDAQLink(uint8_t iDAQLink)
{
...
void
WIB::EnableDAQLink_Lite(uint8_t iDAQLink, uint8_t enable)
{
  // CHeck if we know how to dael with this firmware
  if (!((DAQMode == RCE) || (DAQMode == FELIX))) {
    // Not RCE or FELIX firmware, return
...
void
WIB::ResetWIBAndCfgDTS(uint8_t localClock, uint8_t PDTS_TGRP, uint8_t PDTSsource, uint32_t PDTSAlignment_timeout)
{
  if (DAQMode == UNKNOWN) {
...
void
WIB::CheckedResetWIBAndCfgDTS(uint8_t localClock, uint8_t PDTS_TGRP, uint8_t PDTSsource, uint32_t PDTSAlignment_timeout)
{
  if (DAQMode == UNKNOWN) {
    BUException::WIB_DAQMODE_UNKNOWN e;
    throw e;
  ...
  if (localClock > 0) {
    ...
    reset_check = ((Read("DTS.CONVERT_CONTROL.EN_FAKE") != 1) || (Read("DTS.CCONVERT_CONTROL.LOCAL_TIMESTAMP") != 1) ||
                   (Read("FEMB_CNC.CNC_CLOCK_SELECT") != 1)
                   //                  || (Read("FEMB_CNC.ENABLE_DTS_CMDS") != 1)
                   || (Read("DTS.SI5344.INPUT_SELECT") != 1) || (Read("DTS.SI5344.ENABLE") != 1));
    if (!reset_check) {
    ...
    reset_check = ((Read("DTS.PDTS_TGRP") != PDTS_TGRP) || (Read("FEMB_CNC.CNC_CLOCK_SELECT") != 1) ||
                   (Read("DTS.PDTS_ENABLE") != 1) || (Read("DTS.CDS.LOL") != 0) || (Read("DTS.CDS.LOS") != 0) ||
                   (ReadWithRetry("DTS.SI5344.INPUT_SELECT") != 0) || (ReadWithRetry("DTS.SI5344.LOS") != 0) ||
                   (ReadWithRetry("DTS.SI5344.LOL") != 0) || (ReadWithRetry("DTS.SI5344.ENABLE") != 1) ||
  ...
  if (DAQMode == FELIX) {
    ...
    if ((Read("DAQ.SI5342.ENABLE") == 0) || (Read("DAQ.SI5342.INPUT_SELECT") != 1) || (Read("DAQ.SI5342.LOL") == 1) ||
        (Read("DAQ.SI5342.LOS_XAXB") == 1) || (Read("DAQ.SI5342.LOS_2") == 1)) {
...
void
WIB::StartStreamToDAQ(bool link1_enabled, bool link2_enabled, bool link3_enabled, bool link4_enabled)
{
  if (DAQMode == UNKNOWN) {
    BUException::WIB_DAQMODE_UNKNOWN e;
    throw e;
  ...
  if (DAQMode == FELIX) {
    if (link1_enabled)
      EnableDAQLink_Lite(1, 1);
    if (link2_enabled)
      EnableDAQLink_Lite(2, 1);
  } else {
    if (link3_enabled)
      EnableDAQLink_Lite(3, 1);
    if (link4_enabled)
      EnableDAQLink_Lite(4, 1);
  }
...
void
WIB::FEMBPower(uint8_t iFEMB, bool turnOn)
{
...
bool
WIB::CheckDAQLinkInRange(uint8_t iDAQLink)
{
...
char
WIB::GetDAQLinkChar(uint8_t iDAQLink)
{
  char c = '0';
...
bool
WIB::CheckFEMBInRange(uint8_t iFEMB)
{
...
bool
WIB::CheckFEMBStreamInRange(uint8_t iStream)
{
...
bool
WIB::CheckFEMBCDInRange(uint8_t iCDA)
{
...
void
WIB::SourceFEMB(uint64_t iFEMB, uint64_t real)
{
  if (iFEMB < 1) {
