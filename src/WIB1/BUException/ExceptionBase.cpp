...
void
BUException::exBase::Copy(const exBase& rh) throw()
{
  ...
  } else { // rh was valid, so construct ours
...
void
BUException::exBase::Append(const char* buffer) throw()
{
  ...
  strncpy(descriptionBuffer + descriptionUsed, buffer, appendedSize);
...
const char*
BUException::exBase::Description() const throw()
{
