...
const char*
BUException::exBase::StackTrace() const throw()
{
  if (stackBuffer == NULL)
    return stackError;
  return stackBuffer;
...
void
BUException::exBase::AppendStackLine(const char* line) throw()
{
  // Check for NULL line
  if (line == NULL)
    return;
...
void
BUException::exBase::GenerateStackTrace() throw()
{
  ...
  for (size_t iFunction = 3; iFunction < stackSize; iFunction++) {
    ...
    if ((mangledStart != NULL) && (offsetStart != NULL) && (offsetEnd != NULL) && (mangledStart < offsetStart)) {
      ...
      *offsetEnd = '\0';
      ...
      int demangledRet = 0;
      char* retName = abi::__cxa_demangle(mangledStart, demangledName, &demangledSize, &demangledRet);
