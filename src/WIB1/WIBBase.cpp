...
WIBBase::WIBBase(std::string const& address, std::string const& WIBAddressTable, std::string const& FEMBAddressTable)
  : wib(NULL)
  , FEMBReadSleepTime(0.01)
  , FEMBWriteSleepTime(0.01)
{
...
uint32_t
WIBBase::ReadI2C(std::string const& base_address, uint16_t address, uint8_t byte_count)
{
  ...
  while (ReadWithRetry(base_address + ".DONE") == 0x0) {
    usleep(1000);
  }
  // Run transaction
...
void
WIBBase::WriteI2C(std::string const& base_address,
                  uint16_t address,
                  uint32_t data,
                  uint8_t byte_count,
                  bool ignore_error)
...
Item const*
WIBBase::GetItem(std::string const& str)
{
  return wib->GetItem(str);
}

Item const*
WIBBase::GetFEMBItem(int iFEMB, std::string const& str)
{
  if ((iFEMB > 4) || (iFEMB < 1)) {
...
uint32_t
WIBBase::ReadWithRetry(uint16_t address)
{
  return wib->ReadWithRetry(address);
}
uint32_t
WIBBase::Read(uint16_t address)
{
  return wib->Read(address);
}
uint32_t
WIBBase::ReadWithRetry(std::string const& address)
{
  return wib->ReadWithRetry(address);
}
uint32_t
WIBBase::Read(std::string const& address)
{
  return wib->Read(address);
}

void
WIBBase::WriteWithRetry(uint16_t address, uint32_t value)
{
  wib->WriteWithRetry(address, value);
}
void
WIBBase::Write(uint16_t address, uint32_t value)
{
  wib->Write(address, value);
}
void
WIBBase::WriteWithRetry(std::string const& address, uint32_t value)
{
  wib->WriteWithRetry(address, value);
}
void
WIBBase::Write(std::string const& address, uint32_t value)
{
  wib->Write(address, value);
}
void
WIBBase::Write(uint16_t address, std::vector<uint32_t> const& values)
{
  wib->Write(address, values);
}
void
WIBBase::Write(std::string const& address, std::vector<uint32_t> const& values)
{
  wib->Write(address, values);
}
void
WIBBase::Write(uint16_t address, uint32_t const* values, size_t word_count)
{
  wib->Write(address, values, word_count);
}
void
WIBBase::Write(std::string const& address, uint32_t const* values, size_t word_count)
{
  wib->Write(address, values, word_count);
}

uint32_t
WIBBase::ReadFEMB(int iFEMB, uint16_t address)
{
  if ((iFEMB > 4) || (iFEMB < 1)) {
    BUException::WIB_INDEX_OUT_OF_RANGE e;
    e.Append("In WIBBase::ReadFEMB\n");
...
uint32_t
WIBBase::ReadFEMB(int iFEMB, std::string const& address)
{
  if ((iFEMB > 4) || (iFEMB < 1)) {
    BUException::WIB_INDEX_OUT_OF_RANGE e;
    e.Append("In WIBBase::ReadFEMB\n");
...
void
WIBBase::WriteFEMB(int iFEMB, uint16_t address, uint32_t value)
{
  if ((iFEMB > 4) || (iFEMB < 1)) {
    BUException::WIB_INDEX_OUT_OF_RANGE e;
...
void
WIBBase::WriteFEMB(int iFEMB, std::string const& address, uint32_t value)
{
  if ((iFEMB > 4) || (iFEMB < 1)) {
    BUException::WIB_INDEX_OUT_OF_RANGE e;
    e.Append("In WIBBase::WriteFEMB\n");
...
void
WIBBase::WriteFEMBBits(int iFEMB, uint16_t address, uint32_t pos, uint32_t mask, uint32_t value)
{
  if ((iFEMB > 4) || (iFEMB < 1)) {
    BUException::WIB_INDEX_OUT_OF_RANGE e;
    e.Append("In WIBBase::WriteFEMB\n");
...
void
WIBBase::EnableADC(uint64_t iFEMB, uint64_t enable)
{
  if (iFEMB > 4 || iFEMB < 1) {
    BUException::WIB_INDEX_OUT_OF_RANGE e;
  ...
  if (bool(enable))
    FEMB[iFEMB - 1]->Write(0x03, 0x00);
  else
    FEMB[iFEMB - 1]->Write(0x03, 0xFF);
  usleep((useconds_t)FEMBWriteSleepTime * 1e6);
}
