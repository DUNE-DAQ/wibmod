...
static void
ParseIHexLine(std::string line, std::vector<uint8_t>& data, uint32_t& upperAddr)
{
  if (line.empty()) {
...
void
LoadIHexFile(std::vector<uint8_t>& data, const char* file)
{
  std::string line;
...
firmwareFromIntelHexFile(std::string const& iHexFileName)
{
...
void
WIB::ReadFlash(std::string const& fileName, uint8_t update_percentage)
{
...
void
WIB::EraseFlash(bool print_updates)
{
  if (print_updates) {
...
void
WIB::ProgramFlash(std::string const& fileName, uint8_t update_percentage)
{
...
void
WIB::WriteFlash(std::vector<uint32_t> flashData, uint8_t update_percentage)
{
  ...
  for (size_t currentBlockStartIndex = 0; currentBlockStartIndex < flashData.size();) {
    ...
    for (size_t iBlockWord = 0; iBlockWord < blockSize; iBlockWord++) {
      ...
      WriteWithRetry(blockRegMapAddress + iBlockWord, flashData[currentBlockStartIndex + iBlockWord]);
    }
    // Do the block write
...
void
WIB::CheckFlash(std::vector<uint32_t> flashData, uint8_t update_percentage)
{
  bool print_updates = false;
  if (update_percentage < 100) {
    print_updates = true;
