...
uint32_t
AddressTable::Read(uint16_t address)
{
  return io->Read(address);
}
uint32_t
AddressTable::ReadWithRetry(uint16_t address)
{
  return io->ReadWithRetry(address);
}

void
AddressTable::Write(uint16_t address, uint32_t data)
{
  io->Write(address, data);
}
void
AddressTable::WriteWithRetry(uint16_t address, uint32_t data)
{
  io->WriteWithRetry(address, data);
}

void
AddressTable::Write(uint16_t address, std::vector<uint32_t> const& values)
{
  io->Write(address, values);
}
void
AddressTable::Write(uint16_t address, uint32_t const* values, size_t word_count)
{
  io->Write(address, values, word_count);
}

uint32_t
AddressTable::Read(std::string registerName)
{
...
uint32_t
AddressTable::ReadWithRetry(std::string registerName)
{
  std::map<std::string, Item*>::iterator itNameItem = nameItemMap.find(registerName);
  if (itNameItem == nameItemMap.end()) {
    BUException::INVALID_NAME e;
...
void
AddressTable::Write(std::string registerName, uint32_t val)
{
  std::map<std::string, Item*>::iterator itNameItem = nameItemMap.find(registerName);
  if (itNameItem == nameItemMap.end()) {
    BUException::INVALID_NAME e;
...
void
AddressTable::WriteWithRetry(std::string registerName, uint32_t val)
{
  std::map<std::string, Item*>::iterator itNameItem = nameItemMap.find(registerName);
  if (itNameItem == nameItemMap.end()) {
    BUException::INVALID_NAME e;
...
void
AddressTable::Write(std::string registerName, std::vector<uint32_t> const& values)
{
  Write(registerName, values.data(), values.size());
}
void
AddressTable::Write(std::string registerName, uint32_t const* values, size_t word_count)
{
  std::map<std::string, Item*>::iterator itNameItem = nameItemMap.find(registerName);
  if (itNameItem == nameItemMap.end()) {
    BUException::INVALID_NAME e;
