...
#define DEFINE_CONVERSION_CLASS(name, body)                                                                            \
  class itemconv_##name : public ItemConversion                                                                        \
  {                                                                                                                    \
    STRIP(body)                                                                                                        \
  };
...
ItemConversion*
ItemConversion::FromString(std::string convstring)
{
  unordered_map<std::string, std::string> params;
  int i = 0;
  while (i < (int)convstring.size() && convstring[i] != ' ')
    i++;
  ...
  while (i < (int)convstring.size()) {
    ...
    int pair_end = i;
    int j = pair_start;
    while (j < (int)convstring.size() && convstring[j] != '=')
      j++;
    std::string key = std::string(convstring.c_str(), pair_start, j - pair_start);
    std::string value = std::string(convstring.c_str(), j + 1, pair_end - (j + 1));
    params[key] = value;
  }
