...
std::vector<data_8b10b_t>
WIB::ReadOutCDLinkSpyBuffer()
{
...
std::vector<data_8b10b_t>
WIB::ReadDAQLinkSpyBuffer(uint8_t iDAQLink, uint8_t trigger_mode)
{
  ...
  while (!ReadWithRetry(base + "EMPTY")) {
    ...
    for (size_t iWord = 0; iWord < 4; iWord++) {
      ret.push_back(data_8b10b_t((k_data >> iWord) & 0x1, (data >> (iWord * 8) & 0xFF)));
    }
    // mark word as read
    Write(base + "DATA", 0x0);
  }
