...
ADC_ASIC_reg_mapping::ADC_ASIC_reg_mapping()
  : BITS()
{
}

void
ADC_ASIC_reg_mapping::set_ch(uint8_t chip,
                             uint8_t chn,
                             uint8_t d,
                             uint8_t pcsr,
                             uint8_t pdsr,
                             ...
                             uint8_t tstin)
{
  ...
  unsigned long chn_reg =
    (tmp_d & 0xF0) + ((pcsr & 0x01) << 3) + ((pdsr & 0x01) << 2) + ((slp & 0x01) << 1) + ((tstin & 0x01) << 0);
...
void
ADC_ASIC_reg_mapping::set_global(uint8_t chip,
                                 uint8_t f4,
                                 uint8_t f5,
                                 uint8_t slsb,
                                 uint8_t res4,
...
void
ADC_ASIC_reg_mapping::set_chip(uint8_t chip,
                               uint8_t d,
                               uint8_t pcsr,
                               uint8_t pdsr,
                               uint8_t slp,
                               ...
                               uint8_t f3)
{
  ...
  set_global(chip, f4, f5, slsb, res4, res3, res2, res1, res0, clk0, clk1, frqc, engr, f0, f1, f2, f3);
}

void
ADC_ASIC_reg_mapping::set_board(uint8_t d,
                                uint8_t pcsr,
                                uint8_t pdsr,
                                uint8_t slp,
                                uint8_t tstin,
...
std::bitset<1152>
ADC_ASIC_reg_mapping::get_bits() const
{
  return BITS;
}

void
ADC_ASIC_reg_mapping::print() const
{
