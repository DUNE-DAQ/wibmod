...
void
WIB::ConfigWIBFakeData(bool enableFakeFEMB1,
...
uint8_t
WIB::GetFEMBStreamSource(uint8_t iFEMB, uint8_t iStream)
{
...
void
WIB::SetFEMBStreamSource(uint8_t iFEMB, uint8_t iStream, bool real)
{
  CheckFEMBStreamInRange(iStream);
  std::string base = "FEMB0.DAQ.FAKE_CD.RX_DATA_SOURCE";
  base[4] = GetFEMBChar(iFEMB);
...
void
WIB::SetFEMBFakeCOLDATAMode(uint8_t iFEMB, uint8_t iCD, bool mode)
{
...
uint8_t
WIB::GetFEMBFakeCOLDATAMode(uint8_t iFEMB, uint8_t iCD)
{
  std::string base("FEMB0.DAQ.FAKE_CD.CD0.");
  base[4] = GetFEMBChar(iFEMB);
  base[20] = GetFEMBCDChar(iCD);
