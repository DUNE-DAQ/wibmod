...
uint32_t
WIB::ReadLocalFlash(uint16_t address)
{
  // load the address
...
std::vector<uint32_t>
WIB::ReadLocalFlash(uint16_t address, size_t n)
{
...
void
WIB::WriteLocalFlash(uint16_t address, uint32_t data)
{
  // load the address
  Write("SYSTEM.FLASH.ADDRESS", address);
...
void
WIB::WriteLocalFlash(uint16_t address, std::vector<uint32_t> const& data)
{
