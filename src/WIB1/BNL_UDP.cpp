...
static std::string
dump_packet(uint8_t* data, size_t size)
{
  //  printf("Err: %p %zu\n",data,size);
  std::stringstream ss;
...
void
BNL_UDP::Setup(std::string const& address, uint16_t port_offset)
{
  ...
  if (getaddrinfo(address.c_str(), NULL, NULL, &res)) {
    ...
    if (address.size() <= 5 && 1 == std::count(address.begin(), address.end(), '.')) {
      ...
      if (strCrate.size() != 0 && strSlot.size() != 0) {
        ...
        if ((((crate > 0) && (crate < 7)) || 0xF == crate) && (((slot > 0) && (slot < 7)) || 0xF == slot)) {
...
void
BNL_UDP::WriteWithRetry(uint16_t address, uint32_t value, uint8_t retry_count)
{
  while (retry_count > 1) {
    try {
      // Do the write
...
void
BNL_UDP::Write(uint16_t address, uint32_t value)
{
  ...
  ssize_t sent_size = 0;
  if (send_size != (sent_size = send(writeSocketFD, &packet, send_size, 0))) {
    // bad send
  ...
  if (writeAck) {
    ssize_t reply_size = recv(writeSocketFD, buffer, buffer_size, 0);
...
void
BNL_UDP::Write(uint16_t address, std::vector<uint32_t> const& values)
{
  Write(address, values.data(), values.size());
}
void
BNL_UDP::Write(uint16_t address, uint32_t const* values, size_t word_count)
{
  ...
  ////////  //Create a pointer to 16bit words that points to the parts of buffer that are after the first 32bit word
  ///(WIB key)
...
uint32_t
BNL_UDP::ReadWithRetry(uint16_t address, uint8_t retry_count)
{
  uint32_t val;
  while (retry_count > 1) {
    try {
...
BNL_UDP::Read(uint16_t address)
{
  ...
  ssize_t send_size = sizeof(packet);
  ssize_t sent_size = 0;
  if (send_size != (sent_size = send(readSocketFD, &packet, send_size, 0))) {
    // bad send
    BUException::SEND_FAILED e;
    if (sent_size == -1) {
  ...
  ssize_t reply_size = recv(readSocketFD, buffer, buffer_size, 0);
  ...
  //  }
  uint32_t ret = ((uint32_t(buffer[2]) << 24) | (uint32_t(buffer[3]) << 16) | (uint32_t(buffer[4]) << 8) |
                  (uint32_t(buffer[5]) << 0));
  return ret;
}
...
void
BNL_UDP::ResizeBuffer(size_t size)
{
