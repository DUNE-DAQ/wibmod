...
void
WIB::InitializeDTS(uint8_t PDTSsource, uint8_t clockSource, uint32_t PDTSAlignment_timeout)
{
  // Disable the PDTS
  ...
  if (ReadWithRetry("DTS.SI5344.LOS") || ReadWithRetry("DTS.SI5344.LOL")) {
    // Disable SI5344 outputs
    WriteWithRetry("DTS.SI5344.ENABLE", 0);
    // Disable SI5344
    WriteWithRetry("DTS.SI5344.RESET", 1);
  ...
  char const* const PDTSStates[] = { "W_RST", "W_LINK", "W_FREQ", "W_ADJUST", "W_ALIGN", "W_LOCK", "W_PHASE", "W_RDY",
                                     "RUN",   "0x9",    "0xA",    "0xB",      "ERR_R",   "ERR_T",  "ERR_P",   "0xF" };
