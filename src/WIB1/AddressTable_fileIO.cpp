...
void
AddressTable::LoadFile(std::string const& fileName, std::string const& prefix, uint16_t offset)
{
...
void
AddressTable::ProcessLine(std::string const& line, size_t lineNumber, std::string const& prefix, uint16_t offset)
{
  ...
  for (; itToken != tokens.end(); itToken++) {
    switch (iToken) {
      case 0: { // keep name out of everyone else's scope
        // Assign name
