...
void
WIB::WriteDTS_SI5344(uint16_t address, uint32_t value, uint8_t byte_count)
{
  WriteI2C("DTS.SI5344.I2C", address, value, byte_count);
}
uint32_t
WIB::ReadDTS_SI5344(uint16_t address, uint8_t byte_count)
{
...
void
WIB::SetDTS_SI5344Page(uint8_t page)
{
...
uint8_t
WIB::GetDTS_SI5344AddressPage(uint16_t address)
{
  return uint8_t((address >> 8) & 0xFF);
}

void
WIB::LoadConfigDTS_SI5344(std::string const& fileName)
{
...
void
WIB::SelectSI5344(uint64_t input, bool enable)
{
